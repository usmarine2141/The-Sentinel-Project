...
class Ping(object):
	...
	def print_start(self):
		msg = "\nPinging %s (%s) with %d bytes of data" % (self.destination, self.dest_ip, self.packet_size)
	...
	def print_success(self, delay, ip, packet_size, ip_header, icmp_header):
		if ip == self.destination:
			...
			msg = "Response from %s: bytes=%d icmp_seq=%d ttl=%d time=%.1fms" % (from_info, packet_size, icmp_header["seq_number"], ip_header["ttl"], delay)
	...
	def print_exit(self):
		msg = "\nPing statistics for %s:" % (self.destination)
